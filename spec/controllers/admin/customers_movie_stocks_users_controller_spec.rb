...
describe Admin::CustomersMovieStocksUsersController do
  fixtures :all
  render_views
  ...
  it "create action should redirect when model is valid" do
    ...
    post :create
    response.should redirect_to(admin_customer_movie_stock_user_url(assigns[:customer]))
  end
  ...
  it "update action should redirect when model is valid" do
    CustomersMovieStocksUser.any_instance.stubs(:valid?).returns(true)
    put :update, :id => CustomersMovieStocksUser.first
    response.should redirect_to(admin_customer_movie_stock_user_url(assigns[:customer_movie_stock_user]))
  end

  it "destroy action should destroy model and redirect to index action" do
    customer_movie_stock_user = CustomersMovieStocksUser.first
    delete :destroy, :id => customer_movie_stock_user
    response.should redirect_to(admin_customers_movie_stocks_users_url)
